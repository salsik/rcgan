...
class CycleGANTraining:
    def __init__(self,
                 logf0s_mcep_normalization_zero,
                 logf0s_mcep_normalization_traineduser,
                 logf0s_mcep_normalization_newuser,
                 coded_sps_0_norm,
                 coded_sps_user_norm,
                 coded_sps_newuser_norm,
                 model_checkpoint,
                 validation_S_dir,
                 output_S_dir,
        ...
        # add the default tts zero norm file and the new user norm file
        self.datasets.append(self.loadPickleFile(coded_sps_0_norm))
        self.datasets.append(self.loadPickleFile(coded_sps_user_norm))
       ...
       #self.coded_sps_A_mean = mcep_normalization['mean_A']
        ...
        self.enc_0 = Encoder("enc0").to(self.device)
        self.dec_0 = Decoder("dec0").to(self.device)
       ...
       # self.encoder_List.append(Encoder("Enc2").to(self.device))
        ...
        self.file_name = 'log_store_my_rcgan.txt'
          ...
          #  dataset = trainingDataset(datasetA=self.dataset_A,
           #                           datasetB=self.dataset_B,
            ...
            ### do we need a cycle here and how to apply it
                cycle_A = self.decoder_List[0](encoded_A)
            ...
            ### i think no need for identity right now
                ...
                identity_A =  self.decoder_List[0](self.enc_0(real_A))
            ...
            ### should check the dimension before
                ...
                generator_loss = generator_loss_d_fake_0 + generator_loss_d_fake_A + \
                                 radial_loss_lambda  * radial_loss + cycle_loss_lambda* cycleLoss + identity_loss_lambda* identiyLoss
            ...
            ### i think we should not apply generater again
               ...
               # d_fake_B = self.discriminator_B(generated_B)
                ...
                if num_iterations % 50 == 0:
                    ...
                    store_to_file = "Iter:{}\t Generator Loss:{:.4f} Discrimator Loss:{:.4f} \t radial:{:.4f} G_cyc:{:.4f} G_id:{:.4f}".format(
                       num_iterations, generator_loss.item(), d_loss.item(), radial_loss, cycleLoss, identiyLoss)
                    print("Iter:{}\t Generator Loss:{:.4f} Discrimator Loss:{:.4f} \t radial:{:.4f} G_cyc:{:.4f} G_id:{:.4f}".format(
                        num_iterations, generator_loss.item(), d_loss.item(), radial_loss, cycleLoss, identiyLoss))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logf0s_mcep_normalization_zero', type=str,
                        help="Cached location for log f0s normalized", default=logf0s_mcep_normalization_default)
    parser.add_argument('--logf0s_mcep_normalization_newuser', type=str,
                        help="Cached location for mcep normalization", default=logf0s_mcep_normalization_newuser)
    parser.add_argument('--coded_sps_0_norm', type=str,
                        help="mcep norm for data A", default=coded_sps_0_norm)
    parser.add_argument('--coded_sps_user_norm', type=str,
                        help="mcep norm for data A", default=coded_sps_user_norm)
    ...
    logf0s_mcep_normalization_zero = argv.logf0s_mcep_normalization_zero
    logf0s_mcep_normalization_newuser = argv.logf0s_mcep_normalization_newuser
    #coded_sps_A_norm = argv.coded_sps_A_norm
    coded_sps_0_norm = argv.coded_sps_0_norm
    coded_sps_user_norm = argv.coded_sps_user_norm
    ...
    if not os.path.exists(logf0s_mcep_normalization_zero) or not os.path.exists(logf0s_mcep_normalization_newuser):
        print(
    ...
    cycleGAN = CycleGANTraining(logf0s_mcep_normalization_zero=logf0s_mcep_normalization_zero,
                                logf0s_mcep_normalization_traineduser=logf0s_mcep_normalization_traineduser,
                                logf0s_mcep_normalization_newuser=logf0s_mcep_normalization_newuser,
                                coded_sps_0_norm=coded_sps_0_norm,
                                coded_sps_user_norm=coded_sps_user_norm,
                                coded_sps_newuser_norm=coded_sps_newuser_norm,
